...
def _safe_prompt_flags(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "instruction_shape": any(term in lowered for term in ("policy", "instruction", "override", "developer", "system", "role", "jailbreak")),
        "command_execution_shape": any(term in lowered for term in ("shell", "command", "execute", "script", "python", "bash", "terminal")),
        ...
        "path_boundary_shape": any(term in lowered for term in ("path", "file", "directory", "traversal", "workspace")),
...
def _facts_from_prompt(text: str, *, case_id: str = "prompt.case", origin: str = "user", trust: str = "trusted", surface: str = "prompt") -> NormalizedFacts:
    flags = _safe_prompt_flags(text)
    flags["origin_labels"] = [origin]
    flags["trust_labels"] = [trust]
    requested: dict[str, str] = {}
    if flags.get("destructive_action_shape"):
        requested = {"tool": "admin_action", "capability": "admin_action", "action": "destructive_update"}
    elif flags.get("command_execution_shape"):
        requested = {"tool": "execute_code", "capability": "execute_code", "action": "execute"}
    elif flags.get("network_request_shape"):
        requested = {"tool": "network_request", "capability": "network_request", "action": "external_request"}
    elif flags.get("path_boundary_shape"):
        requested = {"tool": "read_file", "capability": "read_file", "action": "read_resource"}
    return NormalizedFacts(
        ...
        features=flags,
        requested=requested,
        policy={"allowed_tools": ("read_task", "summarize_note"), "allowed_capabilities": ("read_task", "summarize_note"), "allowed_sinks": ("local_audit", "user_response"), "trusted_origins": ("user", "host_policy")},
    )
...
def _provider_status(args: argparse.Namespace) -> int:
    sdk_available = CopilotSdkJudgeProvider.import_available() if args.provider == "copilot_sdk" else False
    payload = {
    ...
    if sdk_available and args.provider_calls_enabled:
        ...
        try:
            payload["status"] = CopilotSdkJudgeProvider.auth_status(copilot_home=args.copilot_home)
        except Exception as exc:
            payload["status_error"] = str(exc)
    if args.json:
        print(json.dumps(payload, indent=2, sort_keys=True))
    else:
        print(f"provider={payload['provider']} sdk_available={payload['sdk_available']} copilot_home={payload['copilot_home']}")
    return 0
    ...
    if args.live_call:
        ...
        if not args.provider_calls_enabled:
            ...
            try:
                ...
                result = build_fides_judge("copilot_sdk", provider_config=config).judge(facts)
...
def _rule_engine_for_ids(rule_ids: list[str] | None) -> RuleEngine | None:
    ...
    rules = tuple(rule for rule in load_rules(rules_root()) if rule.id in selected or rule.name in selected)
...
def _judge_one(args: argparse.Namespace) -> int:
    ...
    fides_decision = evaluate_stack(facts, StackName.RULES_PLUS_FIDES, fides_judge=judge, rule_engine=rule_engine)
    payload = {
    ...
    if args.input_format == "jsonl":
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip():
                ...
                rows.append({"id": str(item.get(args.id_field, len(rows))), "text": str(item.get(args.text_field, ""))})
    elif args.input_format == "csv":
        with path.open(newline="", encoding="utf-8") as handle:
            for item in csv.DictReader(handle):
                rows.append({"id": str(item.get(args.id_field, len(rows))), "text": str(item.get(args.text_field, ""))})
    else:
        rows = [{"id": str(i), "text": text} for i, text in enumerate(path.read_text(encoding="utf-8").splitlines()) if text.strip()]
    if args.max_cases is not None:
        rows = rows[: int(args.max_cases)]
    results = []
    counts = {"allow": 0, "quarantine": 0, "block": 0}
    rule_engine = _rule_engine_for_ids(args.rule_id)
    judge = _judge_for_args(args, [f"prompt.{row['id']}" for row in rows]) if args.mode == "warden-plus-fides" else None
    for row in rows:
        facts = _facts_from_prompt(row["text"], case_id=f"prompt.{row['id']}", origin=args.origin, trust=args.trust, surface=args.surface)
        stack = StackName.RULES_PLUS_FIDES if args.mode == "warden-plus-fides" else StackName.YARA_RULES
        decision = evaluate_stack(facts, stack, fides_judge=judge, rule_engine=rule_engine)
        counts[decision.decision.value] += 1
        results.append({"id": row["id"], "prompt_included": False, "prompt_length": len(row["text"]), "decision": decision.to_dict()})
    payload = {"schema_version": "canaryweave_fides.bench_scan.v1", "prompt_rows_included": False, "total_prompts": len(rows), "decision_counts": counts, "results": results}
    _write_json(args.output, payload)
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    provider = subparsers.add_parser("provider", help="Inspect optional FIDES provider integrations")
    ...
    one.add_argument("--rule-id", action="append", default=None, help="Limit WARDEN to one or more rule IDs/names")
    one.add_argument("--fides-mode", default="disabled", choices=["disabled", "test_double", "copilot_sdk"])
    one.add_argument("--test-verdict", default="unsafe", choices=["safe", "unsafe", "uncertain"])
    ...
    scan.add_argument("--input-format", default="jsonl", choices=["jsonl", "csv", "txt"])
    ...
    scan.add_argument("--rule-id", action="append", default=None, help="Limit WARDEN to one or more rule IDs/names")
    scan.add_argument("--mode", default="warden", choices=["warden", "warden-plus-fides"])
    ...
    eval_parser.add_argument("--config", type=Path, default=None, help="Eval YAML config (default: packaged data/evals/smoke.yaml)")
    eval_parser.add_argument("--datasets-config", type=Path, default=None, help="Dataset catalog YAML (default: packaged conf/datasets.yaml)")
    eval_parser.add_argument("--stacks-config", type=Path, default=None, help="Stack catalog YAML (default: packaged conf/stacks.yaml)")
    ...
    eval_parser.add_argument("--copilot-home", type=Path, default=None, help="Private Copilot SDK home for quarantined FIDES judge")
    eval_parser.add_argument("--public-report", action="store_true", help="Write aggregate public-safe report")
    eval_parser.add_argument("--fail-on-missing-optional-dataset", action="store_true", help="Treat skipped optional datasets as an eval error instead of reporting an explicit skip")
    ...
    if args.command == "eval":
        ...
        configured_mode = args.fides_mode or (str(getattr(loaded, "fides_mode", FidesJudgeMode.DISABLED).value) if loaded else FidesJudgeMode.DISABLED.value)
        fides_rules = getattr(loaded, "fides_test_double_evidence_rules", ()) if loaded is not None else ()
        if stacks is None:
            run_config = EvaluationRunConfig(adapters=adapters, iterations=iterations, fides_mode=configured_mode)
        else:
        ...
        if configured_mode == FidesJudgeMode.COPILOT_SDK.value:
            ...
            fides_judge = build_fides_judge("copilot_sdk", provider_config=provider_config)
        report = run_evaluation(run_config, fides_judge=fides_judge, private_review_csv=args.private_review_csv)
        missing = [result for result in report.get("adapter_results", []) if result.get("status") == "skipped_missing_local_path"]
