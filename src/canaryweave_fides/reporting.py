...
def _security_metrics_for_stack(
    rows: list[dict[str, Any]], stack: str
) -> dict[str, float | int]:
...
def _blocked_case_keys(
    rows: list[dict[str, Any]], stack: str, attack_only: bool = False
...
def _attack_keys(
    ...
    return {
        _case_key(row)
        for row in rows
        if row.get("stack") == stack and row.get("case_kind") == "attack"
    }


def _group_counts(rows: list[dict[str, Any]], field: str) -> dict[str, dict[str, int]]:
    grouped: dict[str, dict[str, int]] = defaultdict(
        lambda: {"total": 0, "blocked_or_quarantined": 0}
    )
    for row in rows:
...
def _coverage_entry(
    rule_ids: set[str], hit_count: int, all_rule_ids: set[str]
) -> dict[str, Any]:
...
def _rule_coverage(
    rows: list[dict[str, Any]], all_rule_ids: set[str]
) -> dict[str, Any]:
    ...
    by_dataset_category: dict[str, dict[str, set[str]]] = defaultdict(
    ...
    return {
        ...
        "dataset_rule_coverage": {
            dataset: sorted(ids) for dataset, ids in sorted(by_dataset.items())
        },
        ...
        "rule_coverage_by_category": _sorted_dict(
            {
                category: _coverage_entry(
                    ids, by_category_hits[category], total_rule_ids
                )
                ...
                dataset: _sorted_dict(
                    {
                        category: _coverage_entry(
                            ids,
                            by_dataset_category_hits[dataset][category],
                            total_rule_ids,
                        )
...
def _decisions_by_case(
    rows: list[dict[str, Any]], stacks: Iterable[str]
) -> dict[tuple[Any, Any, Any, Any], dict[str, str]]:
    wanted = set(stacks)
...
def _disagreement_matrix(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for decisions in by_case.values():
        if all(stack in decisions for stack in _DIAGNOSTIC_STACKS):
            joint_patterns[
                "|".join(decisions[stack] for stack in _DIAGNOSTIC_STACKS)
            ] += 1
...
def _incremental_metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    fides_blocked_attacks = _blocked_case_keys(
        rows, "rules_plus_fides", attack_only=True
    )
    fides_allowed_attacks = _allowed_attack_keys(rows, "rules_plus_fides")
    attack_keys = (
        _attack_keys(rows, "rules_plus_fides")
        or _attack_keys(rows, "yara_rules")
        or _attack_keys(rows, "regex_baseline")
    )
    ...
    fides_called_rows = [
        row
        for row in fides_rows
        if row.get("fides_verdict") not in _NOT_CALLED_FIDES_VERDICTS
    ]
    provider_calls = sum(int(row.get("provider_calls") or 0) for row in fides_rows)
    latencies = [
        float(row["latency_ms"])
        for row in fides_called_rows
    ...
    return {
        ...
        "warden_incremental_catch_rate_vs_regex_misses": (
            round(warden_incremental / regex_allowed_count, 4)
            if regex_allowed_count
            else 0.0
        ),
        "fides_incremental_catches_vs_warden": fides_incremental,
        "fides_incremental_catch_rate_vs_warden_misses": (
            round(fides_incremental / warden_miss_count, 4)
            if warden_miss_count
            else 0.0
        ),
        "fides_incremental_catch_rate_vs_all_attacks": (
            round(fides_incremental / attack_count, 4) if attack_count else 0.0
        ),
        "remaining_misses_after_rules_plus_fides": len(fides_allowed_attacks),
        "remaining_miss_rate_after_rules_plus_fides": (
            round(len(fides_allowed_attacks) / attack_count, 4) if attack_count else 0.0
        ),
...
def _case_fact_context(case_result: Mapping[str, Any]) -> dict[str, Any]:
    safe_features = (
        dict(case_result.get("safe_features") or {})
        if isinstance(case_result.get("safe_features"), Mapping)
        else {}
    )
    policy_context = (
        dict(case_result.get("policy_context") or {})
        if isinstance(case_result.get("policy_context"), Mapping)
        else {}
    )
    requested = {
        "tool": safe_features.get("requested_tool") or safe_features.get("tool"),
        "capability": safe_features.get("requested_capability")
        or safe_features.get("capability"),
        "action": safe_features.get("requested_action") or safe_features.get("action"),
        "sink": safe_features.get("requested_sink")
        or safe_features.get("sink")
        or safe_features.get("target_sink"),
    }
...
def _missing_prerequisite_summary(run_report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "available": available,
        "reason": (
            None if available else "no required_fields metadata present in case results"
        ),
...
def _finalize_expected_rule_entry(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    entry["expected_rule_hit_rate"] = (
        round(int(entry["expected_rule_hits"]) / denominator, 4) if denominator else 0.0
    )
    for key in (
        "unique_expected_rule_ids",
        "unique_hit_rule_ids",
...
def _record_expected_rule_entry(
    entry: dict[str, Any], expected: tuple[str, ...], observed: set[str]
) -> None:
    entry["total_cases"] += 1
    if not expected:
    ...
    entry["unique_expected_rule_ids"] = (
        set(entry["unique_expected_rule_ids"]) | expected_set
    )
    if hit_ids:
    ...
    if missed_ids:
        entry["unique_missed_rule_ids"] = (
            set(entry["unique_missed_rule_ids"]) | missed_ids
        )
...
def _expected_rule_evidence(run_report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    by_dataset_category: dict[str, dict[str, dict[str, Any]]] = defaultdict(
        lambda: defaultdict(_empty_expected_rule_entry)
    )
    ...
    for case_result in run_report.get("case_results", []):
        ...
        _record_expected_rule_entry(
            by_dataset_category[dataset][category], expected, observed
        )
    ...
    return {
        ...
        "reason": (
            None
            if available
            else "no expected_rule_ids metadata present in case results"
        ),
        **_finalize_expected_rule_entry(overall),
        "by_dataset": _sorted_dict(
            {
                key: _finalize_expected_rule_entry(value)
                for key, value in by_dataset.items()
            }
        ),
        "by_category": _sorted_dict(
            {
                key: _finalize_expected_rule_entry(value)
        ...
        "by_dataset_and_category": _sorted_dict(
            {
                dataset: _sorted_dict(
                    {
                        category: _finalize_expected_rule_entry(value)
...
def _record_fact_entry(entry: dict[str, Any], case_result: Mapping[str, Any]) -> None:
    ...
    if _present(
        safe_features.get("requested_capability")
        or safe_features.get("capability")
        or safe_features.get("requested_tool")
    ):
        entry["requested_capability_present_count"] += 1
    if _present(
        safe_features.get("requested_sink")
        or safe_features.get("sink")
        or safe_features.get("target_sink")
    ):
...
def _finalize_fact_entry(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in _FACT_FLAG_KEYS:
        entry[f"{key}_ratio"] = (
            round(int(entry[f"{key}_count"]) / total, 4) if total else 0.0
        )
    for key in (
        "requested_capability_present",
        "requested_sink_present",
        "mapped_category",
        "dataset_native_fallback",
    ):
        entry[f"{key}_ratio"] = (
            round(int(entry[f"{key}_count"]) / total, 4) if total else 0.0
        )
...
def _safe_fact_completeness(run_report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    by_dataset_category: dict[str, dict[str, dict[str, Any]]] = defaultdict(
        lambda: defaultdict(_empty_fact_entry)
    )
    ...
    return {
        **_finalize_fact_entry(overall),
        "by_dataset": _sorted_dict(
            {key: _finalize_fact_entry(value) for key, value in by_dataset.items()}
        ),
        "by_category": _sorted_dict(
            {key: _finalize_fact_entry(value) for key, value in by_category.items()}
        ),
        "by_dataset_and_category": _sorted_dict(
            {
                dataset: _sorted_dict(
                    {
                        category: _finalize_fact_entry(value)
                        for category, value in categories.items()
                    }
                )
                for dataset, categories in by_dataset_category.items()
...
def _finalize_false_positive_entry(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    entry["false_positive_rate"] = (
        round(int(entry["false_positive_decisions"]) / total, 4) if total else 0.0
    )
    return entry
...
def _false_positive_diagnostics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    by_dataset_category: dict[str, dict[str, dict[str, Any]]] = defaultdict(
        lambda: defaultdict(_empty_false_positive_entry)
    )
    ...
    for row in rows:
        ...
        _record_benign_decision(
            by_dataset[dataset], is_false_positive=is_false_positive
        )
        _record_benign_decision(
            by_category[category], is_false_positive=is_false_positive
        )
        _record_benign_decision(
            by_dataset_category[dataset][category], is_false_positive=is_false_positive
        )
    ...
    return {
        ...
        "false_positive_rate": _finalize_false_positive_entry(overall)[
            "false_positive_rate"
        ],
        "by_stack": _sorted_dict(
            {
                key: _finalize_false_positive_entry(value)
                for key, value in by_stack.items()
            }
        ),
        "by_dataset": _sorted_dict(
            {
        ...
        "by_dataset_and_category": _sorted_dict(
            {
                dataset: _sorted_dict(
                    {
                        category: _finalize_false_positive_entry(value)
                        for category, value in categories.items()
                    }
                )
                for dataset, categories in by_dataset_category.items()
...
def build_public_report(
    run_report: Mapping[str, Any], *, all_rule_ids: Iterable[str] | None = None
) -> dict[str, Any]:
    ...
    known_rule_ids = (
        {str(rule_id) for rule_id in all_rule_ids}
        if all_rule_ids is not None
        else _default_rule_ids()
    )
    ...
    return {
        ...
        "maintainability_metrics": {
            ...
            "rule_coverage_by_dataset_and_category": coverage[
                "rule_coverage_by_dataset_and_category"
            ],
        },
