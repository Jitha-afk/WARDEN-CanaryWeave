...
def test_canary_boundary_crossing_hits_only_unauthorized_sink():
    cases = smoke_cases()
    bad_decision = _engine().evaluate(cases["canary_boundary_crossing"].trace, cases["canary_boundary_crossing"].policy)
    good_decision = _engine().evaluate(cases["canary_allowed_sink"].trace, cases["canary_allowed_sink"].policy)
...
def test_text_structure_rules_detect_hidden_and_untrusted_instruction_shapes():
    cases = smoke_cases()
    hidden = _engine().evaluate(cases["hidden_unicode_structure"].trace, cases["hidden_unicode_structure"].policy)
    instruction_shape = _engine().evaluate(cases["untrusted_instruction_shape"].trace, cases["untrusted_instruction_shape"].policy)
    assert "HiddenUnicodeStructure" in [hit.rule_name for hit in hidden.hits]
    assert "UntrustedInstructionShape" in [hit.rule_name for hit in instruction_shape.hits]
