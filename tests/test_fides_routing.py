...
def _facts(
    case_id: str = "case.route.001", text: str = "marker present"
) -> NormalizedFacts:
...
def _engine(
    condition: str = "$foo and $bar", judge: str = _DEFAULT_JUDGE
) -> RuleEngine:
    ...
    judge = StaticFidesJudge(
        {
            facts.case_id: FidesJudgeResult(
                verdict="unsafe", reason_codes=("judge.unsafe",)
            )
        }
    )
...
def test_disabled_rule_fides_keeps_default_allow_behavior():
    decision = evaluate_stack(
        _facts(), StackName.RULES_PLUS_FIDES, rule_engine=_engine()
    )
...
def test_multiple_pending_rule_fides_checks_call_judge_once():
    ...
    decision = evaluate_stack(
        facts, StackName.RULES_PLUS_FIDES, fides_judge=judge, rule_engine=engine
    )
