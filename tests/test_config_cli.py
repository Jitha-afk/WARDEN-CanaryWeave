...
def test_load_eval_config_builds_adapters_and_reports_optional_skips():
    ...
    assert [adapter.dataset_id for adapter in loaded.adapters] == [
        "synthetic",
        "asb",
        "agentdefensebench",
    ]
    assert loaded.iterations == 50
    assert [stack.value for stack in loaded.stacks] == [
        "no_guard",
        "regex_baseline",
        "yara_rules",
        "rules_plus_fides",
    ...
    statuses = {
        adapter.dataset_id: adapter.load().status.value for adapter in loaded.adapters
    }
...
def test_cli_eval_uses_config_and_can_skip_missing_optional_datasets(tmp_path, capsys):
    ...
    statuses = {
        result["dataset_id"]: result["status"] for result in report["adapter_results"]
    }
    assert statuses["synthetic"] == "loaded"
    assert statuses["asb"] == "skipped_missing_local_path"
    assert statuses["agentdefensebench"] == "skipped_missing_local_path"
...
def test_cli_eval_iterations_override_works_when_invoked_as_script(
